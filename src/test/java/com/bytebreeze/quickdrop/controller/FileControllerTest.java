...
class FileControllerTest {
	...
	@Test
	void getFile_whenUploadDirIsIllegal_throwsInvalidPathException() throws Exception {
		// inject an illegal uploadDir that Paths.get() cannot parse
		Field uploadDirField = FileController.class.getDeclaredField("uploadDir");
		uploadDirField.setAccessible(true);
		uploadDirField.set(controller, "://not-a-valid-path");

		assertThrows(
				InvalidPathException.class,
				() -> controller.getFile("anything.txt"),
				"Expected an InvalidPathException when uploadDir contains illegal characters");
	}
}
