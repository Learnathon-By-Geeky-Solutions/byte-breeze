...
public class SSLCommerzPaymentService implements PaymentService {
  ...
  public String getPaymentUrl(ParcelBookingRequestDTO parcelBookingRequestDTO, User sender) {
    ...
    if (responseEntity.getStatusCode() == HttpStatus.OK) {
      ...
      return extractRedirectUrl(
          responseEntity.getBody(), parcelBookingRequestDTO.getPaymentMethod());
    } else {
    ...
    if (hash_verified) {
      ...
      if (!json.isEmpty()) {
        SSLCommerzValidatorResponse resp =
            SSLCommerzUtil.extractValidatorResponse(
        ...
        if (resp.status.equals("VALID") || resp.status.equals("VALIDATED")) {
          ...
          if (merchantTrnxnId.equals(resp.tran_id)
              && (Math.abs(
                      Double.parseDouble(merchantTrnxnAmount)
